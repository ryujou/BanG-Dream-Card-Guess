...
- 新增成绩榜 `/scores`，可实时查看排队小游戏排行榜和最近成绩
...
成绩榜位于 `/scores`，用于查看排队小游戏成绩。页面会连接本地实时推送接口，新成绩提交后会自动刷新排行榜和最近成绩，适合现场投屏或给工作人员单独打开。
...
| 成绩榜 | `http://127.0.0.1:5173/scores` | 实时查看排队小游戏排行榜和最近成绩 |
...
| `GET /api/queue-scores/events` | 实时推送排队小游戏排行榜和最近成绩 |
