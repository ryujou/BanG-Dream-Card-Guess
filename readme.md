...
- 接入开源音符射手 `/note-shooter`，玩家可扫码直接游玩完整射击小游戏
...
- 自己玩模式会生成自玩页二维码。
- 会生成音符射手二维码，适合玩家等待猜角色时游玩。
- 支持填写 Wi-Fi 名称、密码和加密方式，点击“生成 Wi-Fi 码”后生成 Wi-Fi 二维码。
- Wi-Fi 二维码不会边输入边刷新，避免输入时卡顿。

### 音符射手

音符射手位于 `/note-shooter`，适合玩家等待猜角色时扫码游玩。旧入口 `/queue` 会兼容打开同一个页面，玩家设备只需要和主机在同一个局域网。

- 玩家进入页面后选择难度和关卡开始游戏。
- 移动鼠标或手指控制角色，按住屏幕持续发射音符。
- 游戏结算沿用开源音符射手项目的原有逻辑，排行榜和统计写入本机 `data/note-shooter-scores.json`。
- 项目素材已随页面放入本地静态资源目录，音符射手可在断网局域网内运行。
- 玩法直接接入 [zfkdiyi/bangdream](https://github.com/zfkdiyi/bangdream) 的开源音符射手。

### 成绩榜

成绩榜位于 `/scores`，用于查看本地小游戏成绩。页面会连接本地实时推送接口，新成绩提交后会自动刷新排行榜和最近成绩，适合现场投屏或给工作人员单独打开。
...
| 音符射手 | `http://127.0.0.1:5173/note-shooter` | 玩家等待时扫码游玩开源音符射击小游戏 |
| 成绩榜 | `http://127.0.0.1:5173/scores` | 实时查看本地小游戏排行榜和最近成绩 |
...
- 音符射手：`http://127.0.0.1:5173/note-shooter`
...
6. 等待的玩家也可以扫码打开 `http://192.168.1.23:5173/note-shooter` 游玩音符射手。
...
| 音符射手 | - | 接入 [zfkdiyi/bangdream](https://github.com/zfkdiyi/bangdream) 的开源音符射手 |
...
| `GET /api/queue-scores` | 返回本地小游戏排行榜和最近成绩 |
| `GET /api/queue-scores/events` | 实时推送本地小游戏排行榜和最近成绩 |
| `POST /api/queue-scores` | 提交本地小游戏用户名、分数和游玩时长 |
| `GET /api/note-shooter-scores` | 返回音符射手本地排行榜和最近成绩 |
| `GET /api/note-shooter-scores/events` | 实时推送音符射手排行榜和最近成绩 |
| `DELETE /api/note-shooter-scores` | 删除单条音符射手成绩，需要提交成绩 ID 和主持密码 |
...
│   ├── queue-scores.json       # 旧本地小游戏成绩，本地运行生成
│   └── note-shooter-scores.json # 音符射手成绩，本地运行生成
├── public/                     # 静态资源
