...
export function createFakeTimerService() {
    let round = null;
    let autoNext = null;
    let roundStarts = 0;
    return {
        ...
        setTimeout(callback, _delayMs) {
            autoNext = callback;
            return { stop: () => { if (autoNext === callback)
                    autoNext = null; } };
        },
        tick() {
            if (!round)
            ...
            if (round.leftSeconds <= 0)
                round.onTimeout();
            else
                round.onTick(round.leftSeconds);
        },
        flushAutoNext() {
