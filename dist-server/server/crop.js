...
export function scoreFacePolicy(crop, faceBoxes, mode) {
    ...
    for (const face of faceBoxes) {
        ...
        if (mode === "avoid" && ratio > 0)
            score -= ratio * 800;
        if (mode === "prefer" && ratio > 0.1)
            score += ratio * 600;
        if (mode === "only")
            score = ratio > 0.3 ? score + ratio * 500 : score - 200;
    }
    return score;
}
export function expandedFaceZone(face) {
    const label = face.label || faceLabel(face);
    const config = label === "eyes"
