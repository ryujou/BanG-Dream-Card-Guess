...
import { AUTH_COOKIE, CSRF_COOKIE, isAuthenticated, verifyPassword, buildAuthCookie, buildCsrfCookie, createAuthSession, createCsrfToken, getAuthToken, getCookie, revokeAuthSession } from "./auth.js";
import { readCommunityData, writeCommunityData } from "./community.js";
import { parseRequestPayload, readRequestBody, normalizeNoteShooterPlayerId, normalizeQueueUsername, } from "./scores.js";
import { createProductionServices } from "./services/production.js";
import { createInitialGameState, markRoundLoadFailed, markRoundLoading, markRoundPlaying, } from "./game/state.js";
...
const { cardProvider, cropService, timerService, scoreStore, qrcodeService, networkService, } = services;
...
const server = createServer(async (req, res) => {
    ...
    if (url.pathname === "/api/health")
        return sendJson(res, publicHealthSnapshot());
    ...
    if (url.pathname === "/api/network")
        return sendJson(res, networkService.networkState(req));
    ...
    if (url.pathname === "/api/queue-scores" && req.method === "GET")
        return sendJson(res, scoreStore.queueScoreState());
    if (url.pathname === "/api/queue-scores/events")
    ...
    if (url.pathname.startsWith("/note-shooter-api/"))
        return scoreStore.handleNoteShooterApi(url, req, res);
    if (url.pathname === "/api/note-shooter-scores" && req.method === "GET")
        return sendJson(res, scoreStore.noteShooterScoreState());
    if (url.pathname === "/api/note-shooter-scores/events")
    ...
    if (url.pathname.startsWith("/img/")) {
        ...
        if (legacyFile.startsWith(legacyBase) && legacyStat?.isFile())
...
async function handleCommand(ws, command, payload) {
    ...
    switch (command) {
        ...
        case "reveal":
            clearAutoNext();
            stopTimer();
            applyPureCommand(command, safePayload);
            break;
        case "selfGuess":
            judgeSelfGuess(safePayload.guess);
            break;
        case "hideAnswer":
            applyPureCommand(command, safePayload);
            break;
...
function finishRound(result) {
    ...
    if (settings.autoNext) {
        ...
        timerService.startAutoNextTimer(settings.autoNextDelay, () => {
            if (token === roundToken && (game.status === "revealed" || game.status === "finished"))
                startRound();
        });
    }
}
...
async function updateSettings(next) {
    ...
    broadcast();
    if (difficultyChanged || modeChanged) {
        preparedRound = null;
        stopTimer();
        clearAutoNext();
        game.status = "idle";
...
function prepareNextRound() {
    ...
    if (preparedRound && preparedRoundKey === key)
        return;
